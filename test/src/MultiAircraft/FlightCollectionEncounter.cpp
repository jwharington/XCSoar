...
void FlightCollectionEncounter::finalise()
{
  ...
  incursion_tracker.FlushActive();

  terrain_tracker.FlushActive();

  std::cout << "[finalise] writing " << incursion_tracker.completed_incursions.size() << " incursion sets..." << std::endl;
...
std::string FlightCollectionEncounter::get_symbol(const AircraftModel &m) const
{
  ...
  else if (terrain_tracker.IsActive(m.idi))
  {
    return std::string("\033[1;33m@\033[0m");
  }
  else if (incursion_tracker.IsActive(m.idi))
  {
    return std::string("\033[1;35m^\033[0m");
  }
  else if (m.in_flock)
  {
    return std::string("|");
