...
namespace MultiAircraft
{
    namespace
    {
        ...
        using ProcessDiagGust = std::array<double, 10>;
        using MeasurementDiag = std::array<double, 4>;
        using StateDiag = std::array<double, 9>;
        using StateDiagGust = std::array<double, 10>;
        ...
        FlightReconstruction::StateWithUpdraftGust MakeInitialStateGust(const FlatPoint &fp,
                                                                        const TrailPoint &p)
        {
            return FlightReconstruction::get_initial_state_estimate_with_updraft_gust(
                fp.y, fp.x, -p.pos.gps_altitude,
                p.v_tas,
                p.bank_angle.Radians(),
                p.pitch_angle.Radians(),
        ...
        Eigen::Vector3d QuaternionResidual(const FlightReconstruction::StateWithUpdraftGust &predicted,
                                           const FlightReconstruction::StateWithUpdraftGust &actual)
        {
            const auto &q_pred = std::get<1>(predicted.data).get_q();
            const auto &q_actual = std::get<1>(actual.data).get_q();
        ...
        ProcessDiagGust GetBaselineProcessGust()
        {
            ProcessDiagGust out{};
            const auto defaults = FlightReconstruction::GetProcessCovarianceDefaultsWithUpdraftGust();
            for (std::size_t i = 0; i < out.size(); ++i)
                out[i] = defaults[i].second;
            return out;
        }

        StateDiagGust GetBaselineStateGust()
        {
            StateDiagGust out{};
            const auto defaults = FlightReconstruction::GetStateCovarianceDefaultsWithUpdraftGust();
            for (std::size_t i = 0; i < out.size(); ++i)
                out[i] = defaults[i].second;
            return out;
        }
    ...
    // Forward declaration for gust-specific tuning helper
    CovarianceTuningResult TuneFlightReconstructionCovariancesGust(
        ...
        const CovarianceTuningConfig &config)
    {
        ...
        const bool uses_gust_filter = AircraftModel::filter_type == 3 || AircraftModel::filter_type == 4;

        // Delegate to gust-specific tuning if needed
        if (uses_gust_filter)
        {
            return TuneFlightReconstructionCovariancesGust(aircraft, config, t_start);
        }

        std::cout << "[cov-tune] start (base filter)"
        ...
        const std::chrono::steady_clock::time_point &t_start)
    {
        ...
        std::cout << "[cov-tune] start (gust filter)"
                  << " total_aircraft=" << aircraft.size()
                  << " max_flights=" << config.max_flights
                  << " max_points_per_flight=" << config.max_points_per_flight
                  << " max_failures_per_flight=" << config.max_failures_per_flight
        ...
        ResidualPool<4> measurement_pool{};
        ResidualPool<10> process_pool_gust{};
        ResidualPool<10> state_pool_gust{};
        ...
        {
            auto tuned_r = BuildDiagonal<4>(measurement_pool, GetBaselineMeasurement(), config, rng, measurement_labels);
            auto tuned_q = BuildDiagonal<10>(process_pool_gust, GetBaselineProcessGust(), config, rng, process_labels_gust);
            auto tuned_p = BuildDiagonal<10>(state_pool_gust, GetBaselineStateGust(), config, rng, state_labels_gust);

            // Enforce symmetry constraints
            ConstrainEqualPair(tuned_r, 0, 1);
            ConstrainEqualPair(tuned_q, 0, 1);
            ConstrainEqualPair(tuned_p, 0, 1);
            ConstrainEqualTriplet(tuned_q, 7, 8, 9); // attitudes at indices 7,8,9
            ConstrainEqualTriplet(tuned_p, 7, 8, 9); // attitudes at indices 7,8,9
        ...
        std::array<double, 4> tuned_r{};
        std::array<double, 10> tuned_q{};
        std::array<double, 10> tuned_p{};
        std::array<double, 4> prev_r{};
        std::array<double, 10> prev_q{};
        std::array<double, 10> prev_p{};
        bool have_prev_fit = false;
        bool have_fit = false;
        bool converged = false;
        boost::json::array convergence_history;
        ...
        for (std::size_t pass = 1; pass <= max_passes; ++pass)
        {
            ...
            for (const auto *ap : pass_flights)
            {
                ...
                const FlatPoint fp0 = projection.ProjectFloat(it->pos.location) * projection.GetApproximateScale();
                const auto initial_state = MakeInitialStateGust(fp0, *it);

                FlightReconstruction::FilterWithUpdraftGust filter;
                filter.initialise(initial_state, 1.0);

                bool have_previous_state = false;
                FlightReconstruction::StateWithUpdraftGust previous_state;
                std::size_t success_updates_this_flight = 0;
                std::size_t failures_this_flight = 0;
                std::size_t consecutive_failures = 0;
                std::size_t restarts_this_flight = 0;
                ...
                for (; it != trail.end(); ++it)
                {
                        ...
                        consecutive_failures >= config.max_consecutive_failures)
                    {
                        ...
                        {
                            const FlatPoint fp_restart = projection.ProjectFloat(it->pos.location) * projection.GetApproximateScale();
                            const auto restart_state = MakeInitialStateGust(fp_restart, *it);
                            filter.initialise(restart_state, 1.0);
                            have_previous_state = false;
                            consecutive_failures = 0;
                            ++restarts_this_flight;
                    ...
                    if (!have_previous_state)
                    {
                        ...
                        const auto curr_deriv = FlightReconstruction::convert_state(state);
                        for (int i = 0; i < 6; ++i)
                            state_pool_gust[static_cast<std::size_t>(i)].push_back(curr_deriv[i] - init_deriv[i]);

                        // w_g at index 6
                        state_pool_gust[6].push_back(curr_deriv[6] - init_deriv[6]);

                        const Eigen::Vector3d dq = QuaternionResidual(initial_state, state);
                        state_pool_gust[7].push_back(dq.x());
                        state_pool_gust[8].push_back(dq.y());
                        state_pool_gust[9].push_back(dq.z());
                        ++initial_state_samples;
                    ...
                    for (int i = 0; i < 6; ++i)
                        process_pool_gust[static_cast<std::size_t>(i)].push_back(curr_deriv[i] - pred_deriv[i]);

                    // w_g at index 6
                    process_pool_gust[6].push_back(curr_deriv[6] - pred_deriv[6]);

                    const Eigen::Vector3d dq = QuaternionResidual(predicted, state);
                    process_pool_gust[7].push_back(dq.x());
                    process_pool_gust[8].push_back(dq.y());
                    process_pool_gust[9].push_back(dq.z());
                    ++process_samples;
        ...
        const auto process_defaults = FlightReconstruction::GetProcessCovarianceDefaultsWithUpdraftGust();
        const auto measurement_defaults = FlightReconstruction::GetMeasurementCovarianceDefaults();
        const auto state_defaults = FlightReconstruction::GetStateCovarianceDefaultsWithUpdraftGust();
        ...
        const auto t_end = std::chrono::steady_clock::now();
        const auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t_end - t_start).count();
        std::cout << "[cov-tune] done (gust) elapsed_ms=" << elapsed_ms
                  << " measurement_samples=" << measurement_samples
                  << " process_samples=" << process_samples
                  << " initial_state_samples=" << initial_state_samples << "\n";
