...
struct IGCFRInfoDBType
{
  ...
  {
    if ((nullptr != StringFind(other.fr_type, fr_type)) && (nullptr != StringFind(other.fw_version, fw_version)))
    {
      ...
      if (nullptr != line)
      {
        ...
        if (std::regex_match(sline, pieces_match, pattern))
        {
          if (4 == pieces_match.size())
          {
            ...
            strncpy(d.fr_type, pieces_match[2].str().c_str(), sizeof(d.fr_type) - 1);
            strncpy(d.fw_version, pieces_match[3].str().c_str(), sizeof(d.fw_version) - 1);
