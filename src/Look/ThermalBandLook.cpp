...
ThermalBandLook::Initialise(bool _inverse, Color sky_color)
{
  ...
  if (IsDithered()) {
    brush_active.Create(ColorWithAlpha(COLOR_BLACK, alpha));
    brush_inactive.Create(ColorWithAlpha(COLOR_GRAY, alpha/2));

    pen_active.Create(Layout::ScalePenWidth(1), DarkColor(COLOR_GRAY));
    pen_inactive.Create(Layout::ScalePenWidth(1), COLOR_BLACK);
